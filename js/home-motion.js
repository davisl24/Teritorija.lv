(() => {
  ...
  function setupLandingStory() {
    ...
    if (about) {
      about.classList.add('nv-about--story');
      about.innerHTML = `
    ...
    const solutions = document.querySelector('.nv-solutions');
    const list = solutions?.querySelector('.nv-solution-list');
    if (solutions && list) {
      solutions.classList.add('nv-solutions--curated');
      list.innerHTML = `
  ...
  function polishProductVisuals() {
    ...
    const alt = ['ZANO āra sols','ZANO āra galds','ZANO velo statīvs','SAWO velo nojume','SAWO velo servisa stacija','FreeKids rotaļu elements'];
  ...
  function setupProcessTimeline() {
    ...
    const start = () => { if (reduced || paused || !inView || timer) return; timer = window.setInterval(() => activate(activeIndex + 1), 3500); };
    items.forEach((item, index) => {
      const summary = item.querySelector('summary');
      summary?.addEventListener('mouseenter', () => { paused = true; stop(); activate(index); });
      summary?.addEventListener('focusin', () => { paused = true; stop(); activate(index); });
      summary?.addEventListener('click', () => { window.setTimeout(() => activate(index), 0); });
    });
    section.addEventListener('mouseleave', () => { paused = false; start(); });
    section.addEventListener('focusout', (event) => { if (section.contains(event.relatedTarget)) return; paused = false; start(); });
    activate(0);
    setupReveal('.nv-process', .18);
    if (!reduced && 'IntersectionObserver' in window) {
      const observer = new IntersectionObserver(([entry]) => { inView = entry.isIntersecting; if (inView) start(); else stop(); }, { threshold: .25 });
  ...
  function setupInfinitePartners() {
    ...
    const buildCopies = () => {
      ...
      while (track.scrollWidth < viewport.clientWidth * 3) base.forEach((node) => track.appendChild(node.cloneNode(true)));
      normalizeManufacturerLinks(track);
    };
    buildCopies();
    if (reduced) return;
